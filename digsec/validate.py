...
def do_validate(argv):
    ...
    if len(corresponding_rrsig) > 1:
        print_answer_file(corresponding_rrsig_filename)
        error('multiple RRSIGs in the %s file' % corresponding_rrsig_filename)
    ...
    an_rr = rrset[0]
    rrsig = corresponding_rrsig[0].l2()
    for rr in rrset:
        ...
        if rr.clas != rrsig.clas:
            ...
            error('RR.class: %s is different than RRSIG.class: %s' %
                  (rr.clas, rrsig.clas))
        # this is probably not correct for wildcards
        if len(rr.name.split('.')) != rrsig.labels:
            print_answer_file(an_rrset_filename)
            print_answer_file(corresponding_rrsig_filename)
            error('RR.name has different number of labels than RRSIG.labels')
    now = datetime.now()
    ...
    if use_ds:
        ...
        if len(ds_rrset) > 1:
            print_answer_file(dnskey_or_ds_rrset_filename)
            error('multiple DS records in %s' % dnskey_or_ds_rrset_filename)
        ds = ds_rrset[0]
        ...
        if valid:
            ...
            valid = validate_dnskey(dnskey_or_ds_rrset_filename,
                                    ...
                                    ds)
            if valid:
                print('OK DS (%s) with DNSKEY (%d, %s)' % (ds.digest_type,
                                                           ds.keytag,
                                                           ds.algorithm))
            else:
        ...
        if valid:
            ...
            print('ERROR in RRSIG (%s, %s) Validation ! Signature Mismatch' %
                  (rrsig.type_covered, rrsig.algorithm))
            sys.exit(1)
